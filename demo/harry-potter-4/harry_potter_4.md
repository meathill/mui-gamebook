...
"那是什么鬼东西？！"{{ if ron_friendship >= 40 }}罗恩大喊。{{ else }}有人大喊。{{ /if }}
...
每次你看到一个女孩，话就卡在喉咙里。{{ if ron_friendship >= 40 }}罗恩也一样——他问了芙蓉·德拉库尔，差点被她的媚娃血统催眠得说不出话来。{{ /if }}

{{ if ron_friendship >= 40 }}
"我们完蛋了。"罗恩绝望地说。
{{ /if }}
...
"而且，"帕瓦蒂补充道，"我妹妹帕德玛可以做{{ if ron_friendship >= 40 }}罗恩{{ else }}你的朋友{{ /if }}的舞伴。"
...
但至少你有舞伴了。{{ if ron_friendship >= 40 }}她的妹妹帕德玛也愿意做罗恩的舞伴。{{ /if }}
...
{{ if ron_friendship >= 40 }}罗恩的下巴掉到了地上。{{ /if }}
...
勇士们领舞。{{ if ball_partner == "parvati" }}你挽着漂亮的帕瓦蒂{{ /if }}{{ if ball_partner == "luna" }}你和穿着银色长袍的卢娜{{ /if }}{{ if ball_partner != "parvati" && ball_partner != "luna" }}你独自一人{{ /if }}走到舞池中央。

说实话，你跳得很糟糕。你踩了舞伴好几次脚。

但幸好，其他人很快也加入了舞池，你终于可以躲到一边。

赫敏和克鲁姆跳得非常优雅。{{ if ron_friendship >= 40 }}罗恩酸溜溜地盯着他们看。{{ /if }}
...
赫敏开始在图书馆疯狂查阅资料。{{ if ron_friendship >= 40 }}罗恩也在帮忙。{{ /if }}
...
罗恩醒了过来，"我怎么了？全是水……{{ if ron_friendship >= 40 }}哇，那个法国妞在亲你！"{{ else }}离我远点。"{{ /if }}
