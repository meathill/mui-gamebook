...
describe('useGameAnalytics', () => {
  ...
  it('trackOpen calls /api/analytics/open', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/analytics/open', expect.objectContaining({
      method: 'POST',
  ...
  it('trackScene calls /api/analytics/scene', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/analytics/scene', expect.objectContaining({
      method: 'POST',
  ...
  it('trackChoice calls /api/analytics/choice', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/analytics/choice', expect.objectContaining({
      method: 'POST',
  ...
  it('trackComplete calls /api/analytics/complete', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/analytics/complete', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ gameId: 123 }),
    }));
  });
