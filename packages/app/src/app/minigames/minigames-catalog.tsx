...
export const MINIGAMES_PAGE_SIZE = 16;

export async function generateMinigamesMetadata(page: number): Promise<Metadata> {
  const t = await getTranslations('minigames');
  return {
    title: t('pageTitle'),
    description: t('pageDescription'),
    alternates: { canonical: catalogPageHref('/minigames', page) },
  };
}

export async function MinigamesCatalog({ page }: { page: number }) {
  const currentPage = Math.max(1, page);
  const offset = (currentPage - 1) * MINIGAMES_PAGE_SIZE;

  const [minigames, totalCount, t] = await Promise.all([
    getPublicMinigames({ limit: MINIGAMES_PAGE_SIZE, offset }),
    getPublicMinigamesCount(),
    getTranslations('minigames'),
  ]);

  const totalPages = Math.ceil(totalCount / MINIGAMES_PAGE_SIZE);
