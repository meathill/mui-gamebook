...
import { VALIDATE_GAME_PROMPT, VALIDATE_GAME_STEPS } from '@/lib/skills/validate-game';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('validatePageTitle'),
    description: t('validatePageDescription'),
    alternates: { canonical: '/skills/validate-game' },
  };
}

export default async function ValidateGameSkillPage() {
  const t = await getTranslations('skills');
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: t('validateCardTitle'), item: `${baseUrl}/skills/validate-game` },
    ],
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-stone-900 via-stone-800 to-emerald-950 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold mb-6 tracking-tight">{t('validateHeroTitle')}</h1>
          <p className="text-lg text-stone-200 max-w-3xl leading-relaxed mb-4">{t('validateHeroSubtitle')}</p>
          <p className="text-sm text-emerald-300 mb-8">{t('validateOrderNote')}</p>
          <div className="flex flex-wrap gap-4">
            <a
              href="/api/skills/validate-game/skill-md"
              ...
              {t('downloadValidatePack')}
            </a>
            <CopyButton
              text={VALIDATE_GAME_PROMPT}
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto space-y-6">
          {VALIDATE_GAME_STEPS.map((step, index) => (
            <div
              key={step.slug}
