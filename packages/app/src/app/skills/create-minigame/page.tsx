...
import { CREATE_MINIGAME_PROMPT, CREATE_MINIGAME_STEPS } from '@/lib/skills/create-minigame';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('minigamePageTitle'),
    description: t('minigamePageDescription'),
    alternates: { canonical: '/skills/create-minigame' },
  };
}

export default async function CreateMinigameSkillPage() {
  const t = await getTranslations('skills');
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: t('minigameCardTitle'), item: `${baseUrl}/skills/create-minigame` },
    ],
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-stone-900 via-stone-800 to-indigo-950 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold mb-6 tracking-tight">{t('minigameHeroTitle')}</h1>
          <p className="text-lg text-stone-200 max-w-3xl leading-relaxed mb-4">{t('minigameHeroSubtitle')}</p>
          <p className="text-sm text-indigo-300 mb-8">{t('minigameOrderNote')}</p>
          <div className="flex flex-wrap gap-4">
            <a
              href="/api/skills/create-minigame/skill-md"
              ...
              {t('downloadMinigamePack')}
            </a>
            <CopyButton
              text={CREATE_MINIGAME_PROMPT}
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto space-y-6">
          {CREATE_MINIGAME_STEPS.map((step, index) => (
            <div
              key={step.slug}
