...
- **`trigger`** (可选): 触发器配置，当条件满足时跳转到指定场景
  - `condition`: 触发条件，「运算符 + 字面量」的前缀形式（如 `<= 0`、`== true`），与变量当前值比较
  - `scene`: 跳转的场景 ID
  - 注意：当前实现仅对**数字和布尔**变量可靠生效，字符串变量的 trigger 暂不支持
...
当前算术能力边界：每条赋值只支持**一次** `+` 或 `-` 运算（如 `gold = gold + 5`），且两个操作数都必须是数字；不支持 `*`、`/`、链式运算（`a + b + c`）和括号。逗号是语句分隔符，因此字符串值中不能包含逗号。每条赋值必须带 `=`——`(set: courage + 10)` 是**非法**写法，运行时会静默跳过。
...
- 运算符优先级（低到高）：顶层逗号（等价 AND）< `or`/`||` < `and`/`&&` < `not`/`!` < 比较运算符
- 单独写一个变量名（如 `(if: has_key)`）表示真值判断
- `==`/`!=` 是宽松比较（数字 `10` 与字符串 `"10"` 相等）
- 逗号是条件分隔符，因此字符串字面量中不能包含逗号
