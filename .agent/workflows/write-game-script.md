...
在适当位置插入小游戏增强体验。请确保使用正确的 yaml 格式：
...
1. 遍历剧本中所有 YAML 块，找到 `minigame` 配置，根据 prompt 生成完整的小游戏逻辑
...
5. 确保 JS 文件导出符合 DSL 规范的接口：
   ...
   ```
6. `url` 字段暂时留空或填入本地占位符，上传脚本会自动填充。

_> **注意**：上传脚本会自动将 JS 代码内容插入到数据库的 Minigames 表。
...
// turbo
```bash
MUI_ADMIN_PASSWORD=${MUI_ADMIN_PASSWORD} pnpx tsx scripts/upload-game-assets.ts \
...
**Q: 如何让角色形象一致？**
A: 在 `ai.characters` 中详细定义角色，并在 `image` 中引用
