...
- **Ahrefs 超大图 + 孤儿页（2026-09-14）**：8 张封面以 `width=3840` 被抓、单张 1–3.5MB。主因是 `GameCard` 用 `fill` 没写 `sizes`，浏览器按默认 `100vw` 取 srcSet 最大档（Next 默认 `deviceSizes` 最大 3840），~400px 的卡片请求 4K 全图。修复三件套：① `GameCard` 补 `sizes="(max-width:640px)100vw,(max-width:1024px)50vw,400px"`；② `image-loader.ts` 的 `normalizeWidth` 上限 1920（`MAX_IMAGE_WIDTH`）、`normalizeQuality` 缺省 75（`DEFAULT_IMAGE_QUALITY`，原来 undefined 时不拼 quality 参数）；③ `next.config.ts` 收窄 `deviceSizes` 到 1920 封顶，漏写 `sizes` 也不再出现 3840。**新加 `fill` 图必须写 `sizes`**，按实际渲染宽度写。`/open` 在 sitemap 但全站零内链被判 orphan：页脚加 `footer.open` 链接（`prefetch={false}`，遵守 #20 低意向链接约定）+ 首页 `HighlightsSection` 的 `openDev` 卡片 + 关于页 CTA 各加一条 `/open` 内链，中英 i18n 同步。
- **构建期 bake 空目录（2026-09-14，同轮 Ahrefs 修复线上验证时发现）**：部署后 `/`、`/games`、`/sitemap.xml` 显示空目录，sitemap 只有静态条目，但 `/api/games`、`/games/p/2`、`/play/*` 数据正常。根因：`opennextjs-cloudflare build` 先跑标准 `next build`，在纯 Node 里预渲染 ISR 页，此时无请求上下文、`getCloudflareContext()` 必抛错 → `lib/games` 的 catch 吃掉返回 `[]` → 空结果随 `deploy` 的 `populateCache` 原样上传 R2（它只搬运构建产物，不会用线上 bindings 重渲染）。中招三条件：ISR 预渲染 + 静默空值兜底 + 缓存跨部署保留；`generateStaticParams=[]` 的路由（play、`p/2`）和 dynamic API 构建期不执行，所以没事。修复：`lib/games` 无 D1 binding 时回退抓线上公开 `/api/games`（dynamic、直读生产 D1，无循环依赖；`getAllTags`/`getFeaturedGames`/`count` 从同一快照内存推导；抓不到仍返回空，不 fail loud）；`GET /api/games` 补 `limit/offset` 分页（默认全量，保持兼容）。minigames 无公开 API，sitemap 相关条目继续靠重验证自愈。教训：fail silent 是最贵的失败模式——当初 catch 里抛错的话构建直接挂掉，反而上不了线。
- **minigames 公开面下线（2026-09-14）**：可玩性差，不再公开。删 `app/minigames/` 全目录（列表/分页/详情）、`MiniGameCard`、`StandaloneMiniGamePlayer`（仅详情页用）、`lib/minigames.ts`（已无调用方）；sitemap 去静态 `/minigames` 与动态条目；Header 导航、middleware matcher、`catalog-path`、`public-cache` 联动清理。保留：DB 表与 `/api/cms|agent/minigames`（创作链路）、游戏内 `MiniGamePlayer`（走 API 取码，不受影响）、i18n 里 `minigames`/`header.minigames` 死 key（无运行时影响）。旧 `/minigames/*` URL 直接 404，随重爬从索引脱落。
- **noindex-in-sitemap（2026-09-14）**：16 个 `/play/*` 在 sitemap 里，打开却是"游戏未找到"+noindex，还返回 200。双根因：① sitemap 入口条件只是 `published=1`，播放页却要求有正文+解析成功，内容缺失的 spam/下架行落在裂缝里；② 缺失分支 return 200 div 从不 `notFound()`，标准软 404。修复：目录/置顶/tag/推荐/计数查询统一加 `GameContent 非空 EXISTS`（count 与列表同过滤，分页不断）；`getGameBySlug` 三态——真缺失 null→404、D1 故障 throw→500（不进 ISR 缓存）、构建期抓线上 `/api/games/[slug]`（经 `toPlayableGame` 过滤，不泄露 prompt）；`PlayPage` 缺失改 `notFound()`；`getGamesByTag`/`getRelatedGames` 构建回退从快照推导。旧死链进 404 分组属正常脱落。
- **构建失败教训（2026-09-14，承接上条）**：三态里的"查询抛错即 throw"把构建干挂了——我假设构建期 `getCloudflareContext()` 一定抛错，错。CI 日志证明构建期有两种形态：纯 Node 无上下文（抛错，走线上回退，正常）+ 连上一个**空表 D1**（`no such table`，查询期才爆）。后者撞上 throw，直接 `Export encountered an error … exiting the build`。修复：`NEXT_PHASE=phase-production-build` 时查询失败也走线上回退（再抓不到才 null，保证构建通过）；运行时保持 throw 走 500。结论：任何"运行时抛错"的改动，必须先回答"构建期这行代码跑在哪种形态下"。
- **标题页模版 + 图片链路收敛（2026-09-26）**：新增 `title_layout: classic | fullscreen`（DSL frontmatter → parser 四件套 → 编辑器选项 → `TitleScreen` 分支），缺省 classic 零迁移；fullscreen 只管标题态，游玩态 classic 播放器自带白卡（`GamePlayer` 用 `contents` 包装零侵入，禁止嵌套定义组件，会反复 remount）。沉浸标题页补齐作者行/面包屑/评论/推荐（评论放页面正常流，进游戏被 fixed 层盖住 + 锁滚动，天然不可达）。图片三件事：① CF 压缩本来就是通的（实测 2.78MB PNG → 180KB AVIF，`cf-resized: ok`），慢的主因是沉浸零预加载 + 预加载预了原图 URL（与显示的变换 URL 缓存 key 不同，白预）——`usePreload` 改预 `width=1280` 变换 URL 并接入沉浸模式，另收 `redirect` 指向；② 切场景旧图滞留是 `ImmersiveBackground` 的 prevUrl crossfade 写死的，改黑屏 + 新图 onLoad 后淡入；③ `deviceSizes` 收敛到 3 档 `[768, 1280, 1920]`（`imageSizes: []` 合法，全站 `next/image` 都有 `sizes`），opennext howto 的 custom loader 路径保持不动，不迁 `IMAGES` binding（变换已验证 + 避免 Images 产品计费）。页脚 9 链接挤单行的修法：容器 `flex-wrap` + 链接 `whitespace-nowrap`。
- **页脚改多列 + 沉浸页 extras 折叠（2026-09-26，承接上条）**：单行换行只是止血，链接一多还是乱，照 dyqr.me 改成品牌列 + 读故事/创作/支持三分组 + 底部版权栏（`Footer.tsx`，新增 `footer.tagline/groups` 中英 key）。另修上条埋的坑：沉浸分支的评论/推荐放在页面正常流里，进游戏后 `fixed inset-0` 全屏层没有 z-index，定位元素（评论 widget、推荐卡片）按 DOM 顺序盖到游戏画面上。修法不是加 z-index，而是进游戏直接折叠：标题态附加块统一挂 `play-extras` 类，`globals.css` 里 `html[data-immersive="true"] .play-extras { display: none }`——开关与 header/footer 共用同一个 `data-immersive` 判据，评论的 IntersectionObserver 在 display:none 下不会触发，外部脚本也不加载。教训：fixed overlay 无 z-index 时，后续兄弟里的定位元素一定会浮上来；"进游戏隐藏"这类需求优先用已有的模式开关做 CSS 折叠，别指望覆盖顺序。
